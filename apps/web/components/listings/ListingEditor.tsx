...
export function ListingEditor({ mode, role, initialValues, listingId, onSuccessRedirect, heading, backHref }: ListingEditorProps) {
  ...
  function toggleMulti<K extends keyof ListingFormValues>(key: K, item: string) {
    setForm((f) => {
  ...
  }, [form.title, slugManuallyEdited]); // eslint-disable-line react-hooks/exhaustive-deps

  /* Reset subcategory + smart priceUnit when type changes — skip first mount in edit mode */
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <SectionCard title="Core details">
            ...
            <div className="grid grid-cols-5 gap-2">
              {LISTING_TYPES.map((t) => (
                <button key={t.value} type="button"
                  ...
                  disabled={mode === "edit"}
                  className={`flex flex-col items-center gap-1 py-2.5 rounded-xl border-2 transition-all text-center disabled:opacity-60 disabled:cursor-not-allowed ${
                    form.type === t.value
          ...
          <Field label="Slug *" hint="URL-safe identifier — lowercase, hyphens only. Must be unique.">
            ...
            {mode === "edit" && role === "host" && (
              <p className="mt-1 text-[12px] text-text3">URL can&apos;t be changed after publishing.</p>
            )}
          </Field>
