        ...
        {listings.map((l) => {
          ...
          return (
            <Link
              key={l._id}
              href={href}
              className="shrink-0 w-[220px] group rounded-xl border border-[#E2DDD5] overflow-hidden bg-white hover:shadow-md transition-shadow duration-200"
            >
              <div className="relative h-[140px] bg-[#F4F1EC]">
                {l.mainImageUrl ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-[#9C9485] text-sm">
                    No image
                  </div>
                )}
                {l.isVerified && (
                  <span className="absolute top-2 right-2 size-5 rounded-full bg-[#16A34A] flex items-center justify-center">
              ...
              <div className="p-3">
                <p className="text-[13px] font-semibold text-[#16130C] line-clamp-1 group-hover:text-[#E8A020] transition-colors">
                  {l.title}
                </p>
                ...
                {l.pricePerNight != null && l.pricePerNight > 0 && (
                  <p className="text-[12px] font-semibold text-[#16130C] mt-1.5">
                    KSh {l.pricePerNight.toLocaleString()}
                    <span className="font-normal text-[#9C9485]"> / {l.priceUnit || "night"}</span>
                  </p>
                )}
              </div>
            </Link>
