...
export function ExchangeRateWidget() {
  ...
  return (
    <div className="my-8 rounded-2xl border border-[#E2DDD5] overflow-hidden bg-white">
      {/* Header */}
      ...
      {/* Rate cards */}
      <div className="grid grid-cols-3 gap-px bg-[#E2DDD5]">
        {rates.map((r) => (
          <div key={r.code} className="bg-white p-4 text-center">
            <span className="text-[20px]">{r.flag}</span>
            <p className="text-[11px] text-[#9C9485] font-medium mt-1 uppercase tracking-wide">
              1 {r.code}
            </p>
            {r.loading ? (
              <div className="h-6 mt-1 flex justify-center">
                <div className="w-16 h-4 bg-[#F4F1EC] rounded animate-pulse" />
              </div>
            ) : r.rate ? (
              <p className="text-[18px] font-bold text-[#16130C] mt-1">
                KSh {r.rate.toFixed(1)}
              </p>
            ) : (
              <p className="text-[13px] text-[#9C9485] mt-1">—</p>
            )}
          </div>
        ))}
      </div>
      ...
      {/* Converter */}
      <div className="px-5 py-4 bg-[#FAF8F5] border-t border-[#E2DDD5]">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 flex-1">
            <select
              ...
              className="bg-white border border-[#E2DDD5] rounded-lg px-3 py-2 text-[14px] font-medium text-[#16130C] outline-none focus:border-[#E8A020]"
            >
              {rates.map((r) => (
            ...
            <input
              ...
              className="w-24 bg-white border border-[#E2DDD5] rounded-lg px-3 py-2 text-[14px] font-medium text-[#16130C] outline-none focus:border-[#E8A020] text-right"
              min="0"
              step="10"
            />
          </div>
          <span className="text-[#9C9485] text-lg">=</span>
          <div className="flex-1 bg-white border border-[#E8A020]/30 rounded-lg px-4 py-2 text-right">
