  ...
  return (
    <div ref={ref} className={cn("relative flex items-center", className)}>
      ...
      >
        ...
        <span className="text-text2 text-xs">{dial}</span>
        <svg className="w-3 h-3 text-text2" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {/* Dropdown */}
      {open && (
        <div className="absolute left-0 top-full mt-1 z-50 w-64 max-h-60 overflow-auto rounded-xl border border-border bg-white shadow-lg">
          <div className="sticky top-0 bg-white p-2 border-b border-border">
            <input
              ...
              className="w-full rounded-lg border border-border bg-white px-3 py-1.5 text-xs text-text placeholder:text-text3 focus:outline-none focus:border-dark"
              autoFocus
            />
          </div>
          {filtered.map((c) => (
            <button
              ...
              className={cn(
                "flex items-center gap-3 w-full px-3 py-2 text-left text-sm bg-white hover:bg-surface transition-colors",
                c.dial === dial && "bg-amber/10"
              )}
            >
              <span className="text-base">{c.flag}</span>
              <span className="text-text flex-1">{c.name}</span>
              <span className="text-text2 text-xs">{c.dial}</span>
            </button>
          ))}
        </div>
      )}
