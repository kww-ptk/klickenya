  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-border p-5">
        <h2 className="text-[15px] font-bold text-dark mb-4">Daily Page Views</h2>
      ...
      {perListing.length > 0 && (
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          <div className="px-5 py-4 border-b border-[#F0EDE8]">
            <h2 className="text-[15px] font-bold text-dark">Per Listing</h2>
          </div>
              ...
              <tbody className="divide-y divide-[#F0EDE8]">
                {perListing.map((l) => (
                  <tr key={l.slug} className="hover:bg-[#F7F5F2]">
                    <td className="px-5 py-2.5 font-medium text-dark truncate max-w-[200px]">{l.title}</td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* Top referrers */}
        <div className="bg-white rounded-xl border border-border p-5">
          <h2 className="text-[15px] font-bold text-dark mb-3">Top Sources</h2>
            ...
            <div className="space-y-2">
              {topReferrers.map(([domain, count]) => (
                <div key={domain} className="flex items-center justify-between">
                  <span className="text-[13px] text-dark">{domain}</span>
                  <span className="text-[13px] font-semibold text-text3">{count}</span>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Device split */}
        <div className="bg-white rounded-xl border border-border p-5">
          <h2 className="text-[15px] font-bold text-dark mb-3">Devices</h2>
          <div className="flex gap-3">
            {[
              ...
              <div key={d.label} className="flex-1 text-center">
                ...
                <p className="text-[20px] font-bold text-dark">{d.pct}%</p>
