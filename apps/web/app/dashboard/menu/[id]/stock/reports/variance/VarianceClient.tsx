...
const inputCls =
  "border border-[#E2DDD5] rounded-xl px-3 py-2.5 text-[14px] text-[#16130C] bg-white focus:outline-none focus:border-[#E8A020]";
...
export function VarianceClient() {
  ...
  return (
    <div className="space-y-4">
      {/* Range picker */}
      <div className="bg-white rounded-2xl border border-[#E2DDD5] p-4 grid grid-cols-1 sm:grid-cols-[1fr_1fr_auto] gap-3 items-end">
        <div>
          <label className="block text-[11px] font-bold text-[#9C9485] uppercase tracking-wide mb-1">From</label>
          <input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className={`${inputCls} w-full`} />
        </div>
        <div>
          <label className="block text-[11px] font-bold text-[#9C9485] uppercase tracking-wide mb-1">To</label>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="h-11 px-3 rounded-full border border-[#E2DDD5] text-[12px] font-bold text-[#5E5848] hover:border-[#E8A020]"
          >
            Last 7d
          </button>
          <button
      ...
      {/* Headline KPI */}
      <div className="bg-white rounded-2xl border border-[#E2DDD5] p-5 flex items-center justify-between gap-4 flex-wrap">
        <div>
          <p className="text-[11px] font-bold text-[#9C9485] uppercase tracking-wide">Total variance ({from} → {to})</p>
        ...
        <button
          ...
          className="bg-[#E8A020] text-[#16130C] font-bold text-[14px] px-5 h-[48px] rounded-full hover:bg-[#d4911c] disabled:opacity-50"
        >
      ...
      <div className="bg-white rounded-2xl border border-[#E2DDD5] overflow-hidden">
        {loading ? (
          <p className="p-6 text-center text-[14px] text-[#9C9485]">Loading…</p>
        ) : sorted.length === 0 ? (
          <p className="p-6 text-center text-[14px] text-[#9C9485]">No ingredients yet.</p>
        ) : (
                  ...
                  return (
                    <tr key={r.ingredient_id} className="text-[13px]">
                      <td className="px-4 py-3">
                        <p className="font-semibold text-[#16130C]">{r.ingredient_name}</p>
                      ...
                      <td className="px-4 py-3 text-right whitespace-nowrap font-semibold text-[#16130C]">
                      ...
                      <td className="px-4 py-3 text-right">
                        <input
                          ...
                          className="w-24 border border-[#E2DDD5] rounded-lg px-2 py-2 text-[13px] text-right bg-white focus:outline-none focus:border-[#E8A020]"
                        />
                      </td>
