...
export default async function PosLayout({ params, children }: LayoutProps) {
  ...
  let staff: { id: string; menu_id: string; name: string; role: "waiter" | "manager" | "cashier" | "kitchen" | "bar" | "delivery" } | null = null;
  ...
  if (session && session.menu_id === menu.id) {
    ...
    if (row && row.is_active && row.menu_id === menu.id) {
      ...
      staff = {
        ...
        menu_id: row.menu_id,
        name:    row.name,
        role:    row.role as "waiter" | "manager" | "cashier" | "kitchen" | "bar" | "delivery",
      };
