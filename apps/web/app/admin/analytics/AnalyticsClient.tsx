  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-[#E2DDD5] shadow-sm p-6">
        <h2 className="font-display text-[16px] font-bold text-[#16130C] mb-4">Platform Views Over Time</h2>
      ...
      {topByViews.length > 0 && (
        <div className="bg-white rounded-2xl border border-[#E2DDD5] shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-[#F0EDE8]">
            <h2 className="font-display text-[16px] font-bold text-[#16130C]">Top Listings by Views</h2>
          </div>
              ...
              <tbody className="divide-y divide-[#F0EDE8]">
                {topByViews.map((l, i) => (
                  <tr key={l.slug} className="hover:bg-[#F7F5F2]">
                    <td className="px-6 py-2.5 text-[#9C9485]">{i + 1}</td>
                    <td className="px-6 py-2.5 font-medium text-[#16130C] truncate max-w-[200px]">{l.title}</td>
                    <td className="px-6 py-2.5 text-[#9C9485] capitalize">{l.type}</td>
                    <td className="px-6 py-2.5 text-[#9C9485]">{l.city || "—"}</td>
      ...
      {topByConversion.length > 0 && (
        <div className="bg-white rounded-2xl border border-[#E2DDD5] shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-[#F0EDE8]">
            <h2 className="font-display text-[16px] font-bold text-[#16130C]">Top by Conversion Rate <span className="text-[#9C9485] font-normal text-[13px]">(min 10 views)</span></h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-[13px]">
              <thead>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* City breakdown */}
        <div className="bg-white rounded-2xl border border-[#E2DDD5] shadow-sm p-6">
          <h2 className="font-display text-[16px] font-bold text-[#16130C] mb-4">By City</h2>
          <div className="space-y-3">
            {cityBreakdown.map((c) => (
              <div key={c.city}>
                <div className="flex items-center justify-between text-[13px] mb-1">
                  <span className="text-[#16130C] font-medium">{c.city}</span>
                  <span className="text-[#9C9485]">{c.views} views · {c.contacts} contacts</span>
                </div>
                <div className="h-2 rounded-full bg-[#F0EDE8]">
                  <div className="h-full rounded-full bg-[#6B2D8B]/70" style={{ width: `${(c.views / maxCityViews) * 100}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Type breakdown */}
        <div className="bg-white rounded-2xl border border-[#E2DDD5] shadow-sm p-6">
          <h2 className="font-display text-[16px] font-bold text-[#16130C] mb-4">By Listing Type</h2>
          <div className="space-y-3">
            {typeBreakdown.map((t) => (
              <div key={t.type} className="flex items-center justify-between text-[13px] py-2 border-b border-[#F0EDE8] last:border-0">
                <span className="text-[#16130C] font-medium capitalize">{t.type}</span>
                <div className="text-[#9C9485]">
                  <span className="font-semibold text-[#16130C]">{t.views}</span> views · <span className="font-semibold text-[#16130C]">{t.contacts}</span> contacts
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {/* Referrers */}
        <div className="bg-white rounded-2xl border border-[#E2DDD5] shadow-sm p-6">
          <h2 className="font-display text-[16px] font-bold text-[#16130C] mb-4">Traffic Sources</h2>
            ...
            <div className="space-y-2.5">
              {topReferrers.map(([domain, count]) => (
                <div key={domain} className="flex items-center justify-between text-[13px]">
                  <span className="text-[#16130C]">{domain}</span>
                  <span className="font-semibold text-[#9C9485]">{count}</span>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Device split */}
        <div className="bg-white rounded-2xl border border-[#E2DDD5] shadow-sm p-6">
          <h2 className="font-display text-[16px] font-bold text-[#16130C] mb-4">Devices</h2>
          <div className="flex gap-4">
            {[
              ...
              <div key={d.label} className="flex-1 text-center">
                ...
                <p className="text-[22px] font-bold text-[#16130C]">{d.pct}%</p>
