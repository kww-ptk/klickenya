...
export default async function PropertyEnquiriesPage({
  searchParams,
}: {
  ...
  function buildUrl(overrides: Record<string, string | undefined>) {
    ...
    return `/admin/property-enquiries${qs ? `?${qs}` : ""}`;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="font-display text-[22px] lg:text-[28px] font-bold tracking-[-0.03em] text-dark">Property Enquiries</h1>
            <span className="text-[12px] font-semibold text-text3 bg-[#F0EDE8] px-2 py-0.5 rounded-full">{totalCount}</span>
          </div>
          <p className="text-[13px] text-text3 mt-1">Booking and availability enquiries for stay listings — convert to booking, hold, or decline.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#F0EDE8] overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 pt-5 pb-0">
          ...
          {/* Search */}
          <form
            action="/admin/property-enquiries"
            method="get"
            className="relative"
          >
            {status !== "all" && (
          ...
          <table className="w-full text-[13px]">
            ...
            <tbody>
              {items.length === 0 ? (
                ...
                items.map((enq: Record<string, unknown>) => (
                  <tr
