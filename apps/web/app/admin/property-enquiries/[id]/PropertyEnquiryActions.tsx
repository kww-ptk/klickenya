...
export function PropertyEnquiryActions({
  id,
  currentStatus,
  currentNotes,
}: PropertyEnquiryActionsProps) {
  ...
  async function handleStatusUpdate(newStatus: string) {
    ...
    try {
      const res = await fetch(`/api/admin/property-enquiries/${id}/status`, {
        method: "PATCH",
  ...
  async function handleAddNote() {
    ...
    try {
      const combinedNotes = currentNotes
        ? `${currentNotes}\n\n---\n\n${note.trim()}`
        : note.trim();
      const res = await fetch(`/api/admin/property-enquiries/${id}/note`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDraftReply() {
    ...
    try {
      const res = await fetch("/api/admin/ai/draft-reply", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: "property_enquiry", id }),
      });
  ...
  async function handleSendReply() {
    ...
    try {
      const res = await fetch(
        `/api/admin/property-enquiries/${id}/send-reply`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
