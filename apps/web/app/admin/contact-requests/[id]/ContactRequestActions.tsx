...
export function ContactRequestActions({
  ...
  async function handleStatusUpdate(newStatus: string) {
    ...
    try {
      const res = await fetch(`/api/admin/contact-requests/${id}/status`, {
        method: "PATCH",
  ...
  async function handleAddNote() {
    ...
    try {
      const combinedNotes = currentNotes
        ? `${currentNotes}\n\n---\n\n${note.trim()}`
        : note.trim();
      const res = await fetch(`/api/admin/contact-requests/${id}/note`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDraftReply() {
    ...
    try {
      const res = await fetch("/api/admin/ai/draft-reply", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: "contact_request", id }),
      });
  ...
  async function handleSendReply() {
    ...
    try {
      const res = await fetch(`/api/admin/contact-requests/${id}/send-reply`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            ...
            {replyHistory.map((entry, i) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[13px] text-dark font-medium">
                    {entry.subject}
                  </p>
                  <p className="text-[13px] text-text3 leading-relaxed whitespace-pre-wrap">
                    {entry.message}
                  </p>
                </div>
              );
