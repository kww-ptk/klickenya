...
function Field({ label, value }: { label: string; value?: string | null }) {
  return (
    <div>
      ...
      <p className="text-[14px] text-[#16130C]">{value || "—"}</p>
    </div>
  );
}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {(request.ai_score != null || request.ai_summary || (request.ai_flags?.length ?? 0) > 0) && (
            <div className="bg-[#FDF8F0] rounded-2xl border border-[#E8A020] p-6 space-y-4">
              <div className="flex items-center gap-3">
                <h2 className="text-[18px] font-display font-bold text-[#16130C]">AI Analysis</h2>
              ...
              {request.ai_summary && (
                <p className="text-[14px] text-[#5E5848] leading-relaxed">{request.ai_summary}</p>
              )}
              ...
              {request.draft_website && (
                <div>
                  ...
                  <a
                    ...
                    className="text-[14px] text-[#E8A020] hover:underline break-all"
                  >
                    {request.draft_website}
                  </a>
                </div>
          ...
          <div className="bg-white rounded-2xl border border-[#F0EDE8] p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div>
                <p className="text-[12px] text-[#9C9485] uppercase tracking-wider font-medium mb-1">Email</p>
                {request.email ? (
                  <a href={`mailto:${request.email}`} className="text-[14px] text-[#E8A020] hover:underline">
                    {request.email}
                  </a>
                ) : <p className="text-[14px] text-[#16130C]">—</p>}
              </div>
              <div>
                <p className="text-[12px] text-[#9C9485] uppercase tracking-wider font-medium mb-1">Phone</p>
                {request.phone ? (
                  <a href={`tel:${request.phone}`} className="text-[14px] text-[#E8A020] hover:underline">
                    {request.phone}
                  </a>
                ) : <p className="text-[14px] text-[#16130C]">—</p>}
              </div>
              ...
              {request.website_url && (
                <div>
                  <p className="text-[12px] text-[#9C9485] uppercase tracking-wider font-medium mb-1">Website URL</p>
                  <a
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-[#F0EDE8] p-6 space-y-4">
            <h2 className="text-[18px] font-display font-bold text-[#16130C]">Status</h2>
            <StatusBadge status={request.status || "new"} />
            {request.reviewed_at && (
            ...
            {request.sanity_listing_id && (
              <div>
                ...
                <p className="text-[13px] font-mono text-[#16130C] break-all">
                  {request.sanity_listing_id}
                </p>
              </div>
            )}
          ...
          <div className="bg-white rounded-2xl border border-[#F0EDE8] p-6 space-y-3">
            <h2 className="text-[18px] font-display font-bold text-[#16130C]">Quick Actions</h2>
            <Link
              href="/admin/listing-requests"
