...
export default async function AmbassadorDetailPage({
  ...
  const socialHandle = application.social_handle as string | null;
  const socialLink =
    socialHandle && socialHandle.startsWith("@")
      ? `https://instagram.com/${socialHandle.slice(1)}`
      : null;
          ...
          <div className="bg-white rounded-2xl border border-[#F0EDE8] p-6 space-y-4">
            ...
            <div className="pt-2 space-y-2">
              ...
              {application.status !== "rejected" && (
                <button
                  type="button"
                  className="w-full px-4 py-2 text-[13px] font-medium rounded-lg bg-[#EF4444]/10 text-[#EF4444] hover:bg-[#EF4444]/20 transition-colors"
                  disabled
                >
                  Reject
                </button>
          ...
          <div className="bg-white rounded-2xl border border-[#F0EDE8] p-6 space-y-3">
            ...
            {application.email && (
              ...
              >
                ...
                Email {application.name || "applicant"}
              </a>
            )}
          </div>
        </div>
