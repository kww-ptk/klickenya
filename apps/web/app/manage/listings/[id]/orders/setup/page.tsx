...
/**
 * /manage/listings/[id]/orders/setup — ordering SETTINGS.
 *
 * The live queue owns /orders, because that is what an owner means when they
 * say "orders" and it is what they open many times a day. Configuration —
 * which channels are on, the WhatsApp number, the floor plan — is a rare
 * visit and sits one level in.
...
export default async function EatOrdersPage({ params }: PageProps) {
  ...
  if (!user) redirect(`/login?returnTo=/manage/listings/${id}/orders/setup`);
